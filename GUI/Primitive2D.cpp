...
CPrimitive2D::CPrimitive2D(ID3D11Device* const Device, ID3D11DeviceContext* const DeviceContext) :
...
void CPrimitive2D::CreateRectangle(const SInt2& Size, const SFloat4& Color, float Roundness, bool bFlatBottom)
{
	...
	//Roundness = min(max(Roundness, 0.0f), 1.0f); // relative roundness
	Roundness = max(Roundness, 0.0f); // absolute roundness
	...
	if (Roundness > 0)
	{
		...
		//float Radius{ Roundness * RadiusMax }; // relative radius
		float Radius{ Roundness }; // absolute radius
		...
		if (bFlatBottom)
		{
			m_vVertices.emplace_back(SVertex(SFloat4(0		, -Radius - InnerSize.y			, 0, 1), Color));
			m_vVertices.emplace_back(SVertex(SFloat4(Radius	, -Radius - InnerSize.y			, 0, 1), Color));
			m_vVertices.emplace_back(SVertex(SFloat4(0		, -Radius - InnerSize.y	- Radius, 0, 1), Color));
			m_vVertices.emplace_back(SVertex(SFloat4(Radius	, -Radius - InnerSize.y	- Radius, 0, 1), Color));

			uint32_t IndexOffset{ (uint32_t)(4 * 3 + KDetail * 3 * 2) };
			m_vIndices.emplace_back((uint32_t)(IndexOffset + 0));
			m_vIndices.emplace_back((uint32_t)(IndexOffset + 1));
			m_vIndices.emplace_back((uint32_t)(IndexOffset + 2));
			m_vIndices.emplace_back((uint32_t)(IndexOffset + 1));
			m_vIndices.emplace_back((uint32_t)(IndexOffset + 3));
			m_vIndices.emplace_back((uint32_t)(IndexOffset + 2));
		}
		else
		{
		...
		if (bFlatBottom)
		{
			...
			m_vVertices.emplace_back(SVertex(SFloat4(Radius + InnerSize.x + Radius	, -Radius - InnerSize.y			, 0, 1), Color));
			m_vVertices.emplace_back(SVertex(SFloat4(Radius + InnerSize.x			, -Radius - InnerSize.y	- Radius, 0, 1), Color));
			m_vVertices.emplace_back(SVertex(SFloat4(Radius + InnerSize.x + Radius	, -Radius - InnerSize.y	- Radius, 0, 1), Color));

			uint32_t IndexOffset{ (uint32_t)(4 * 3 + KDetail * 3 * 2 + 4) };
			m_vIndices.emplace_back((uint32_t)(IndexOffset + 0));
			m_vIndices.emplace_back((uint32_t)(IndexOffset + 1));
			m_vIndices.emplace_back((uint32_t)(IndexOffset + 2));
			m_vIndices.emplace_back((uint32_t)(IndexOffset + 1));
...
void CPrimitive2D::CreateImage(const SInt2& Size, const SFloat4& TexCoordRange)
{
	XMFLOAT2 _Size{ (float)Size.X, (float)Size.Y };
	m_vVertices.assign(
		{
...
void CPrimitive2D::CreateLine(const std::vector<SVertex>& vVertices)
{
...
void CPrimitive2D::CreateBuffers()
{
	// Vertex buffer
	{
...
void CPrimitive2D::UpdateBuffers()
{
	...
	// Index buffer
	if (m_vIndices.size() && SUCCEEDED(m_PtrDeviceContext->Map(m_IndexBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &MappedSubresource)))
	{
...
void CPrimitive2D::Destroy()
{
	SAFE_RELEASE(m_VertexBuffer);
	SAFE_RELEASE(m_IndexBuffer);
}

void CPrimitive2D::Draw() const
{
	if (m_vIndices.size())
	{
...
std::vector<SVertex>& CPrimitive2D::GetVertices()
{
	return m_vVertices;
}
